...
//DATA
d3.csv("/static/states/florida/FLSenate2.csv", function (error, data) {
    ...
    //Initial Date
    var mindate = new Date("2017-02-26T00:00:00+00:00");
    ...
    var seriesData = varVote.map(function (name) {
      return {
            ...
            values: data.map(function (d) {
              return {name: name, Date: d[dateVar], value: +d[name], source: d[sourceVar]};
            })};});
...
d3.csv("/static/states/florida/FLSenate2.csv", function(error, data){
  		  ...
  		  var tables = d3.select('#page-wrap').append('table');
...
d3.csv("/static/states/florida/FLSenate2.csv", function(error, data){
  ...
  var topData = data.sort(function(a, b) {
    ...
    var totalSum = [
     d3.mean(topData.map(function(d){ return d["Bill Nelson"]})),
     d3.mean(topData.map(function(d){ return d["Rick Scott"]}))];

     console.log(totalSum)


    var yesNo = ["Bill Nelson", "Rick Scott"]
